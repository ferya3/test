...
## نصب آسان و تعاملی (توصیه‌شده) 🚀

نمی‌خواهی دستی فایل بسازی؟ فقط این را اجرا کن — خودش **آیدی‌ها را از تو می‌پرسد**،
فایل را می‌سازد، وابستگی‌ها را نصب می‌کند و سرویس systemd را برای اجرای خودکار
بعد از هر ری‌استارت راه می‌اندازد:

```bash
bash install.sh
```

موقع اجرا، آیدی‌ها را یکی‌یکی وارد می‌کنی (هر کدام Enter)، و برای پایان یک Enter خالی
می‌زنی. تمام. برای اضافه/حذف آیدی بعداً، فقط دوباره `bash install.sh` را اجرا کن.

> کاربر پیش‌فرض سرویس روی `ubuntu` تنظیم شده است.

---

## اجرای خودکار بعد از هر ری‌استارت سرور (Ubuntu + systemd) — روش دستی
